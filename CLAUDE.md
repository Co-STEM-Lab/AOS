...
| Skill | 用途 | 触发词 |
|-------|------|--------|
| `qian-skill` | 钱学森系统科学方法论——复杂工程诊断 + 总体设计 + 涌现检查 | 多模块/跨服务/重构/性能排查时自动启用 |
| `aos-guardian` | 系统守护——不变式巡检 + 新鲜度漂移 + 修复提案 | "检查 AOS" / "守护" / "维护" |
| `aos-operations` | 操作引导——建原子/建项目/改词汇表/出初稿时自动检查 | 任何 AOS 文件修改操作时自动启用 |
